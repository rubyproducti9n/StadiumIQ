  ...
  return (
    <div className="p-4 bg-white rounded-lg border border-gray-200 shadow-sm space-y-4">
      <h3 className="font-bold text-gray-800 flex items-center gap-1">
        <span>📊</span> Upload Crowd Data (.xlsx)
      </h3>
      ...
      {storageError && (
        <div className="p-2.5 bg-red-50 text-red-700 border border-red-100 rounded text-xs">
          ⚠️ {storageError}
        </div>
      )}
      ...
      {errorMsg && (
        <div className="p-2.5 bg-red-50 text-red-700 border border-red-100 rounded text-xs font-medium">
          ❌ {errorMsg}
        </div>
      )}
      ...
      {/* Success Messages */}
      {successMsg && (
        <div className="p-2.5 bg-green-50 text-green-700 border border-green-100 rounded text-xs font-medium">
          ✅ {successMsg}
        </div>
      )}
                ...
                {previewRows.map((row) => (
                  <tr key={row.match_id} className="hover:bg-gray-50">
                    ...
                    <td className="px-3 py-1.5 font-mono">
                      {row.actual_attendance.toLocaleString()}
                      {row.fanWarning && (
                        <span className="ml-1 inline-flex items-center px-1 rounded-full text-[9px] font-semibold bg-yellow-100 text-yellow-800 border border-yellow-200" title="Fan split totals exceeds actual attendance">
                          ⚠️ warning
                        </span>
                      )}
                    </td>
